...
//оператор rest брат оператора spread(...)(рператора разворота), который использует такой же синтаксис но в других условиях.querySelector
//spread брал сущность и раскладывал на отдельные элементы, то rest отдельные элементы оъединяет в один массива
//rest = оставшиеся элементы

const log = function(a, b, ...rest) { // rest = ..., он оворит что может быть много следующих аргументов(...названиеАргумента);
...
function calcOrDouble (number, basis = 2) { // в стандарте ес6 используют такой способ присваивать параметры по умолчанию
    // basis = basis || 2; // раньше таким образом устанавливали параметры по умолчанию
    console.log(number * basis);
}
calcOrDouble(2, 3);

//rest оператор не поддерживает параметры по умолчанию
